...
const Units = () => {
  ...
  const [selectedUnit, setSelectedUnit] = useState<any>(null); // Selected unit for view/edit
...
const fetchLookups = async () => {
  ...
  // Open modal function
  const openModal = (type: "create" | "edit" | "view", unit: any = null) => {
    setModalType(type);
  ...
  const handleFormSubmit = async (formData: Record<string, any>) => {
    ...
    try {
      ...
      if (modalType === "create") {
        // Create
        await api.post("/configure/units", submitData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        toast.success("Unit saved successfully");
      } else if (modalType === "edit" && selectedUnit?.id) {
          ...
          buttons={[
            {
              ...
              variant: "primary",
              size: "sm",
              show: (r) => hasPermission("unit.view"),
              tooltip: "View",
            },
            {
              icon: faEdit,
              ...
              variant: "secondary",
              size: "sm",
              show: (r) => hasPermission("unit.edit") && !r.deleted_at,
              tooltip: "Edit",
            },
            {
  ...
  return (
    <>
      ...
      >
          ...
          <div className="max-w-8xl mx-auto">
            <div className="bg-white flex flex-col sm:flex-row justify-between items-center p-4 border-b border-gray-200">
              ...
              <Button
                ...
                show={hasPermission("unit.create")}
              >
                Add New
              </Button>
            </div>
        ...
        >
          {modalType === "view" && (
            <DynamicViewTable data={selectedUnit} fields={viewFields} />
          )}

          {(modalType === "create" || modalType === "edit") && (
            <DynamicForm
              ref={formRef}
              data={modalType === "edit" ? selectedUnit : null}
              fields={unitFields}
