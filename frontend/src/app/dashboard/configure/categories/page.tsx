...
const Companies = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState<any>(null); // Selected category for view/edit
  ...
  // Open modal function
  const openModal = (type: "create" | "edit" | "view", category: any = null) => {
    setModalType(type);
  ...
  const handleFormSubmit = async (formData: Record<string, any>) => {
    ...
    try {
      ...
      if (modalType === "create") {
        // Create
        await api.post("/configure/categories", submitData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        toast.success("Category saved successfully");
      } else if (modalType === "edit" && selectedCategory?.id) {
        // Edit: Use POST + _method=PUT for Laravel multipart/form-data
        submitData.append("_method", "PUT");
        console.log(formData);

        await api.post(
          `/configure/categories/${selectedCategory.id}`,
          submitData,
          { headers: { "Content-Type": "multipart/form-data" } }
        );
          ...
          buttons={[
            {
              ...
              variant: "primary",
              size: "sm",
              show: (r) => hasPermission("category.view"),
              tooltip: "View",
            },
            {
              icon: faEdit,
              ...
              variant: "secondary",
              size: "sm",
              show: (r) => hasPermission("category.edit") && !r.deleted_at,
              tooltip: "Edit",
            },
            {
  ...
  return (
    <>
      ...
      >
          ...
          <div className="max-w-8xl mx-auto">
            <div className="bg-white flex flex-col sm:flex-row justify-between items-center p-4 border-b border-gray-200">
              ...
              <Button
                ...
                show={hasPermission("category.create")}
              >
                Add New
              </Button>
            </div>
        ...
        >
          {modalType === "view" && (
            <DynamicViewTable data={selectedCategory} fields={viewFields} />
          )}

          {(modalType === "create" || modalType === "edit") && (
            <DynamicForm
              ref={formRef}
              data={modalType === "edit" ? selectedCategory : null}
