...
const Products = () => {
  ...
  const openModal = (type: "create" | "edit" | "view", product: any = null) => {
    setModalType(type);
  ...
  const handleFormSubmit = async (formData: Record<string, any>) => {
    ...
    try {
      ...
      if (modalType === "create") {
        // Create
        await api.post("/configure/products", submitData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        toast.success("Product saved successfully");
      } else if (modalType === "edit" && selectedProduct?.id) {
          ...
          buttons={[
            {
              ...
              size: "sm",
              tooltip: "View",
              show: (r) => hasPermission("product.view"),
            },
            {
              icon: faEdit,
              ...
              size: "sm",
              tooltip: "Edit",
              show: (r) => hasPermission("product.edit") && !r.deleted_at,
            },
            {
              icon: row.deleted_at ? faTrashRestore : faTrash,
              onClick: (r) =>
                r.deleted_at
                  ? handleForceDelete(r, "/configure/products", "product")
                  : handleSoftDelete(r, "/configure/products", "product"),
              variant: "danger",
              size: "sm",
              show: (r) => hasPermission("product.delete"),
  ...
  return (
    <>
      ...
      >
          ...
          <div className="max-w-8xl mx-auto">
            <div className="bg-white flex flex-col sm:flex-row justify-between items-center p-4 border-b border-gray-200">
              ...
              <Button
                ...
                show={hasPermission("product.create")}
              >
                Add New
              </Button>
            </div>
        ...
        >
          {modalType === "view" && (
            <DynamicViewTable data={selectedProduct} fields={productFields} />
          )}
            ...
            <>
              {
                <DynamicForm
                  ref={formRef}
                  data={modalType === "edit" ? selectedProduct : null}
