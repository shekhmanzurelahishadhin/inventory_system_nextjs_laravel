...
const Customers = () => {
  ...
  const handleFormSubmit = async (formData: Record<string, any>) => {
    ...
    try {
      ...
      if (modalType === "create") {
        // Create
        await api.post("/sales/customers", submitData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        toast.success("Customer saved successfully");
      } else if (modalType === "edit" && selectedCustomer?.id) {
        // Edit: Use POST + _method=PUT for Laravel multipart/form-data
        submitData.append("_method", "PUT");
        console.log(formData);

        await api.post(`/sales/customers/${selectedCustomer.id}`, submitData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
          ...
          buttons={[
            {
              ...
              size: "sm",
              tooltip: "View",
              show: (r) => hasPermission("customer.view"),
            },
            {
              icon: faEdit,
              ...
              size: "sm",
              tooltip: "Edit",
              show: (r) => hasPermission("customer.edit") && !r.deleted_at,
            },
            {
            ...
            {
              icon: faUndo,
              onClick: (r) => handleRestore(r, "/sales/customers", "customer"),
              variant: "success",
              size: "sm",
              show: (r) => r.deleted_at,
              tooltip: "Restore",
  ...
  return (
    <>
      ...
      >
        ...
        >
          {modalType === "view" && (
            <DynamicViewTable data={selectedCustomer} fields={customerFields} />
          )}
            ...
            <>
              {loadingDropdowns ? (
                <div className="p-6 text-center">
                  {/* <FormSkeleton fields={customerFields} mode={modalType} /> */}
                  <DatatableLoader />
                </div>
              ) : (
                <DynamicForm
                  ref={formRef}
                  data={modalType === "edit" ? selectedCustomer : null}
